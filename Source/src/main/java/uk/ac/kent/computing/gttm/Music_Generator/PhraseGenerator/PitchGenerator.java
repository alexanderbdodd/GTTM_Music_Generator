...
public class PitchGenerator extends Generator {
    ...
    public final static double BASE_PROLONGATIONAL_VALUE = 0.65;
    public final static double BASE_TIMESPANREDUCTION_VALUE = 0.51;
    ...
    public List<Map<Beat, Key>> getCandidatePitchChains(GrammarContainer grammar,
        ...
        while (candidateChains == null || candidateChains.isEmpty()) {
            ...
            //create candidate chains of beats which demonstrate preferable prolongational
            //reduction features
            chainNo = 0;
            candidateChains = generateCandidateTimeSpanChains(lastKey, localScale);
    ...
    private boolean filterByGPR3a(Map<Beat, Key> chain,
        ...
        for (Group gr : groupingLevel) {
            ...
            if (gr.getMetricalBeatSpan().size() >= 2) {
                ...
                events.add(new AttackEvent(chain.get(firstBeat), DurationEnum.SIXTEENTH));
    ...
    private boolean assessCadenceRelation(Key start, Key finish, Scale localScale) {
        ...
        if (finish.getNote() == localScale.getDominant()
                && start.getNote() == localScale.getTonic()) {
            return true;
        } else {
            if (start.getNote() == localScale.getDominant()
                    && finish.getNote() == localScale.getTonic()) {
                return true;
            }
    ...
    private boolean assessTimeSpanPreference(Map<Beat, Key> chain, Scale localScale) {
        ...
        for (Branch br : branches) {
            ...
            double parentRating = TimeSpanReductionAnalyser
                    .getInstance().getTSPR2andTSPR3(new AttackEvent(chain.get(parentBranch.getAssociatedBeat()), DurationEnum.EIGHTH),
                            new AttackEvent(chain.get(br.getAssociatedBeat()), DurationEnum.EIGHTH), localScale);
            ...
            for (Branch subBr : br.getChildBranches().values()) {
                double childRating = TimeSpanReductionAnalyser
                        .getInstance().getTSPR2andTSPR3(new AttackEvent(chain.get(parentBranch.getAssociatedBeat()), DurationEnum.EIGHTH),
                                new AttackEvent(chain.get(subBr.getAssociatedBeat()), DurationEnum.EIGHTH), localScale);
    ...
    private Map<Beat, List<Key>> generateProlongationalCandidates(Key lastKey, Scale localScale) {
        ...
        AttackEvent lastEv = new AttackEvent(lastKey, DurationEnum.EIGHTH);
        ...
        for (Branch br : getTopPBranch().getAllSubBranches()) {
            ...
            while (firstKey != null) {
                ...
                AttackEvent childEvent = new AttackEvent(firstKey, DurationEnum.EIGHTH);
    ...
    /**
     * A class for use in testing the private methods of the PitchGenerator.
     * Not for use outside testing.
     */
