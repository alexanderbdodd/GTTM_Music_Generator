...
import uk.ac.kent.computing.gttm.Manipulators.GeneralManipulator;
...
public enum KeyLetterEnum {
    ...
    KeyLetterEnum(int keyNumber, int keyIdentity) {
    ...
    public static KeyLetterEnum[] getPreviousNote(KeyLetterEnum current) {
        ...
        while (!pastNext) {
            if (keysArray.get(position).getKeyNumber() == nextNumber) {
            ...
            if (keysArray.get(position).getKeyNumber() == current.getKeyNumber()) {
                position++;
                continue;
            }
