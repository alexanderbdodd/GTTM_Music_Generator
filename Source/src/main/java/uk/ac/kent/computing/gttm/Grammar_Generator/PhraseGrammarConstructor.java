package uk.ac.kent.computing.gttm.Grammar_Generator;
...
import uk.ac.kent.computing.gttm.Grammar_Elements.GroupingStructure.ProlongationalBaseGroup;
...
public class PhraseGrammarConstructor {
	...
	public GrammarContainer constructPhraseGrammar() {
		...
		System.out.println("Prolongational Reduction allocated");
		...
		System.out.println("Prolongational group allocated");
	...
	/**
	 * Creates a prolongational group. At this stage, the prolongational group spans
	 * the whole metrical structure.
	 *
	 * @param span
	 *            the span of beats for the prolongational group to contain
	 ...
	 * contain one cadence branch matching two of the end point of the pitch beats
	 * list.
	 *
	 * @param pitchBeats
	 *            the list of beats assigned as pitches
	 * @param mContainer
	 *            the metrical structure of the grammar
	...
	private TimeSpanReductionBranch allocateTimeSpanReduction(List<Beat> pitchBeats, MetricalContainer mContainer) {
		...
		// structural ending (cadence branch) as parent of structural beginning (TSRPR
		// 9)
		...
		int middlePos = cadenceEnd.getPosition() % 2 == 0 ? (cadenceEnd.getPosition() / 2) - 1
		...
		if (pitchBeat != structuralBeginning.getAssociatedBeat()) {
			...
			addInbetweenBranches(mContainer.getMetricalBeatsList().indexOf(pitchBeats.get(0)) + 1, bottomPos,
					pitchBeats, mContainer, structuralBeginning);
		...
		if (pitchBeat != null) {
			...
			if (endPos > bottomPos) {
				...
				// add branches inbetween middle point of endBeginning to cadenceStart point, to
				// cadenceStart
	 ...
	 *
	 * @param bottomPos
	 *            the first position to include in the time span reduction
	 * @param endPos
	 *            the position after the last position to include in the time span
	 *            reduction
	 * @param pitchBeats
	 *            the list of beats assigned as pitch instantiation beats
	 * @param mContainer
	 *            the metrical structure of the grammar
	 * @param topBranch
	 *            the top branch of the added branches
	 */
	private void addInbetweenBranches(int bottomPos, int endPos, List<Beat> pitchBeats, MetricalContainer mContainer,
			TimeSpanReductionBranch topBranch) {
		...
		while (endPos >= bottomPos) {
			if (pitchBeats.contains(mContainer.getBeatAtPosition(endPos))) {
				TimeSpanReductionBranch newBranch = new TimeSpanReductionBranch(mContainer.getBeatAtPosition(endPos),
						level);
				level++;
	...
	/**
	 * Used to determine pitch beats which are candidates for forming cadence pitch
	 * beats. A pitch beat is preferred as a cadence if it is near the end of the
	 * span of metrical beats and lands on relatively strong beats
	 *
	 * @param pitchBeats
	 *            the list of beats assigned as pitch beats
	 * @param highestStrength
	 *            the highest metrical structure of the piece
	 * @param pos
	 *            the position from which to search for the cadence beats
	 * @param cadenceEnd
	 *            if true, then the beat is meant to represent the cadence end beat,
	 *            else it represents the cadence start beat
	 *
	...
	/*
	 * Creates a prolongational reduction with the following normative structure: 1.
	 * A prolongational ending 2. A prolongational beginning which is a prolongation
	 * (i.e. not a progression) of the prolongational ending. 3. A right branching
	 * progression as the most important direct elaboration of the prolongational
	 * beginning. 4. A left branching progression as the most important elaboration
	 * of the first element of the cadence.
	 * 
	 * This doesn't produce all possible well formed variations of prolongational
	 * structuring yet.
	 */
	private ProlongationalBranch allocateNormativeProlongationalReduction(List<Beat> pitchBeats,
		...
		ProlongationalBranch pEnding = new ProlongationalBranch(pitchBeats.get(0), 0);
		topPBranch = pEnding;
		appliedBeats.add(pitchBeats.get(0));

		// allocating the first pitch beat as the prolongational beginning branch,
		// which is randomly either a strong or weak prolongation of the prolongational
		// ending.
		Collections.sort(pitchBeats, pitchBeats.get(0));
		...
		ProlongationalBranch pBeginning = new ProlongationalBranch(pitchBeats.get(0), 1, pType);
		...
		// allocate prolongational ending pitches
		if (pEnding.getAssociatedBeat() != cadenceBranch.getCadenceEnd()) {
			ProlongationalBranch cBranch = new ProlongationalBranch(cadenceBranch.getCadenceEnd(), endLevel,
					ProlongationalTypeEnum.WEAK_PROLONGATION);
			endLevel++;
		...
		// the cadence start
		assignRandomIntermittantBranches(appliedBeats, leftCadenceBranch, pitchBeats, pos + 1,
				pitchBeats.indexOf(cBranch));

		// doing the same for the beats inbetween the cadence start and cadence end
		assignRandomIntermittantBranches(appliedBeats, cBranch, pitchBeats,
				pitchBeats.indexOf(cBranch.getAssociatedBeat()) + 1, pitchBeats.indexOf(pEnding));

		// doing the same for the beats after the cadence end branch, if any
		assignRandomIntermittantBranches(appliedBeats, pEnding, pitchBeats,
				pitchBeats.indexOf(pEnding.getAssociatedBeat()) + 1, pitchBeats.size());

		// establishing the most important rightBranching progression of the
		// prolongational beginning
		...
		// adding intermittant beats as child of rightBranchStart. This should be
		// adjusted
	 ...
	 *
	 * @param appliedBeats
	 *            Beats already assigned in the prolongational reduction
	 * @param topBranch
	 *            the top prolongational branch of which to append the intermittant
	 *            branches to
	 * @param pitchBeats
	 *            the list of beats assigned as pitch beats
	 * @param pos
	 *            the first position from which to search for pitches to add to the
	 *            prolongational reduction
	 * @param finalPos
	 *            the position after the last beat to search for beats to add to the
	 *            prolongational reduction
	 */
	private void assignRandomIntermittantBranches(List<Beat> appliedBeats, ProlongationalBranch topBranch,
			List<Beat> pitchBeats, int pos, int finalPos) {
		// creating branches for all the pitch beats inbetween the leftCadenceBranch and
		// the cadence start
		...
		ProlongationalTypeEnum pType = rand.nextBoolean() ? ProlongationalTypeEnum.PROGRESSION
				: ProlongationalTypeEnum.WEAK_PROLONGATION;
	...
	/**
	 * Allocates a grouping structure based on the list of the allocated pitch beats
	 * and the metrical container. All base groups must contain at least three
	 * pitches
	 *
	 * @param mContainer
	 *            the metrical structure
	 * @param pitchBeats
	 *            the list of beats allocated as pitch beats
	 * @return the grouping structure's top group, else null if no suitable
	 *         structure could be found
	 */
	private HighLevelGroup allocateGroupingStructure(MetricalContainer mContainer, List<Beat> pitchBeats) {
	...
	private List<BaseGroup> createBaseGroups(MetricalContainer mContainer, List<Beat> pitchBeats) {
		List<List<Beat>> baseGroups = null;

		while (baseGroups == null || baseGroups.size() < 2) {
			baseGroups = allocateBaseGroupBeats(mContainer, pitchBeats);
		}
	...
	/**
	 * Randomly determines how many pitch beats should be allocated in the grammar
	 *
	 * @param mContainer
	 *            the metrical structure
	...
	/*
	 * Assesses the relative candidacy of the beat as being one on which a pitch
	 * falls on. Returns true if the beat should be considered a pitch beat, and
	 * false if not. This is determined according to various constraints, but there
	 * is a degree of randomness as to how far the constraints will be followed.
	 * 
	 * Takes into account the following constraints: 1. A pitch is more likely to
	 * fall on a strong beat. MPR5a
	 */
	...
	/**
	 * Constructs a randomly generated sequence of beats and adds them to a metrical
	 * container.
	 *
	...
	private MetricalContainer constructMetricalContainer() {
		...
		List<Beat> beats = GrammarUtils.getInstance().constructBeatSpan(metricalDistance, metricalStrength);

		MetricalContainer m = GrammarUtils.getInstance().createMetricalContainer(beats);
		// (m.getMetricalBeatsList().size() < 14) {
		// return constructMetricalContainer();
		// } else {
		// return m;
		// }
