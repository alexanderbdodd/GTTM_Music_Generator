...
public class ScaleConstructor {
    ...
    public final KeyLetterEnum[] validKeys = KeyLetterEnum.values();
        private HashMap<Pair<KeyLetterEnum, ScaleModeEnum>, Pair<KeyLetterEnum, ScaleModeEnum>> relativeMinorMap;
    ...
    public Scale getRelativeMinor(KeyLetterEnum letter) {
        ...
        for (Entry<Pair<KeyLetterEnum, ScaleModeEnum>, Pair<KeyLetterEnum, ScaleModeEnum>> pair 
                : relativeMinorMap.entrySet()) {
            if (pair.getKey().first == letter) {
                return constructScale(pair.getValue().first, pair.getValue().second);
            }
        }
    ...
    public Scale getRelativeMajor(KeyLetterEnum letter) {
        ...
        for (Pair<KeyLetterEnum, ScaleModeEnum> l : relativeMinorMap.keySet()) {
            if (relativeMinorMap.get(l).first == letter) {

                return constructScale(l.first, l.second);
            }
        }
    ...
    public Scale constructScale(KeyLetterEnum scaleRoot, ScaleModeEnum scaleMode) {
        ...
        for (KeyLetterEnum letter : ns) {
            if (!keyNumbers.contains(letter.getKeyPosition())) {
                keyNumbers.add(letter.getKeyPosition());
            }
        }
