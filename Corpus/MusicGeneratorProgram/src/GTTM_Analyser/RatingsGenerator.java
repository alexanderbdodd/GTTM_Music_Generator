...
import Elements.*;
import GTTM_Analyser.Exceptions.EventMapConstructionException;
...
public class RatingsGenerator {
    ...
    /**
     * Generates a rating of how well a given EventStream fits the GrammarContainer structure
     * based on the MetricalStructure analysis rules.
     * @param grammar the GrammarContainer object containing the grammar against
     * which to compare the EventStream object
     * @param stream the EventStream object containing the stream of music which
     * is to be compared with the given GrammarContainer object
     * @return a rating indicating how well the given EventStream object fits
     * the grammar given by the GrammarContainer object. A rating of below 0.5
     * indicates that the EventStream largely violates the metrical grammar structure,
     * and a rating equal to or greater than 0.5 indicates that the EventStream
     * object largely does not violate the given metrical grammar structure. A value of
     * 1.0 indicates that there is no violation of the metrical grammar structure. A
     * value of 0.0 indicates a complete violation of the metrical grammar structure.
     */
    ...
    /**
     * Generates a rating of how well a given EventStream fits the GrammarContainer structure
     * based on the TimeSpanReductionAnalyser analysis rules.
     * @param grammar the GrammarContainer object containing the grammar against
     * which to compare the EventStream object
     * @param stream the EventStream object containing the stream of music which
     * is to be compared with the given GrammarContainer object
     * @return a rating indicating how well the given EventStream object fits
     * the grammar given by the GrammarContainer object. A rating of below 0.5
     * indicates that the EventStream largely violates the time-span reduction grammar structure,
     * and a rating equal to or greater than 0.5 indicates that the EventStream
     * object largely does not violate the given time-span reduction  grammar structure. A value of
     * 1.0 indicates that there is no violation of the time-span reduction grammar structure. A
     * value of 0.0 indicates a complete violation of the time-span reduction grammar structure.
     */
    public double obtainTimeSpanReductionRating(GrammarContainer grammar, EventStream stream) {
      ...
      /**
     * Generates a rating of how well a given EventStream fits the GrammarContainer structure
     * based on the ProlongationalReductionAnalyser analysis rules.
     * @param grammar the GrammarContainer object containing the grammar against
     * which to compare the EventStream object
     * @param stream the EventStream object containing the stream of music which
     * is to be compared with the given GrammarContainer object
     * @return a rating indicating how well the given EventStream object fits
     * the grammar given by the GrammarContainer object. A rating of below 0.5
     * indicates that the EventStream largely violates the prolongational reduction grammar structure,
     * and a rating equal to or greater than 0.5 indicates that the EventStream
     * object largely does not violate the given prolongational reduction  grammar structure. A value of
     * 1.0 indicates that there is no violation of the prolongational reduction grammar structure. A
     * value of 0.0 indicates a complete violation of the prolongational reduction grammar structure.
     */
    ...
    //branchType indicates whether to assess the branches using Prolongation reduction analysis rules or
    //timespan reduction analysis rules. 0 indicates prolognational reduction. 1 indicates time span reduction.
    private double branchAnalyser(List<Branch> branches, Branch topBranch, GrammarContainer grammar,
        ...
        Branch originalParent = topBranch;
    

        double[] ratingsArray = new double[branches.size()];
        int i = 0;

        //going through all branches to collect their ratings
        for (Branch branch : branches) {
            ...
            if (branchType == 0) {
                ...
                parentEv = (AttackEvent) beatMap.get(parent.getAssociatedBeat());
            } else {
                ...
                if (branchType == 0) {
                    ...
                    parentRating = ProlongationalReductionAnalyser.getInstance()
                            .assessPRPR3(parentEv, childEvent, ProlongationalReductionAnalyser.getInstance()
                                    .isRightBranching(branch.getAssociatedBeat(), parent.getAssociatedBeat()),
                                    progression, stream.getLocalScale());
    ...
    /**Checks how well a given EventStream fits the Prolongational branching type
     * structure.
     * 
     * @param grammar the GrammarContainer object containing the grammar against
     * which to compare the EventStream object
     * @param stream the EventStream object containing the stream of music which
     * is to be compared with the given GrammarContainer object
     * @return a ratin between 1.0 and 0.0. A rating greater than 0.5 indicates that the 
     * EventStream largely fits the prolongational branching type structure. A rating less than
     * 0.5 indicates that it largely does not fit the branching type structure.
     */
    ...
    /**
     * Generates a rating to indicate how well the EventStream fits the grammar structure
     * cadence constraints.
        @param grammar the GrammarContainer object containing the grammar against
     * which to compare the EventStream object
     * @param stream the EventStream object containing the stream of music which
     * is to be compared with the given GrammarContainer object
     * @return a rating between 1.0 and 0.0. A rating greater than 0.5 indicates that the
     * EventStream largely fits the cadence constraints. A rating below 0.5 indicates that it
     * largely doesn't fit the cadence constraints.
     */
    ...
    private boolean checkIsCadence(CadencedTimeSpanReductionBranch branch, Scale localScale) {
        ...
        if (endEv == null || startEv == null) {
            ...
            if (startEv.getKeyLetterEnum() == localScale.getDominant() && endEv.getKeyLetterEnum() == localScale.getTonic()) {
                return true;
            } else {
                if (startEv.getKeyLetterEnum() == localScale.getTonic() && endEv.getKeyLetterEnum() == localScale.getDominant()) {
                    return true;
                } else {
                    return false;
                }
    ...
    /**
     * Constructs a map of Beat objects against Event objects. Beat objects mapped against
     * a null value indicates that it is a continuation of the last Event mapped by the sequence
     * of Beat objects.
      @param grammar the GrammarContainer object containing the grammar against
     * which to compare the EventStream object
     * @param stream the EventStream object containing the stream of music which
     * is to be compared with the given GrammarContainer object
     * @return the constructed map of Beat objects against Event objects
     */
    public Map<Beat, Event> createEventMap(GrammarContainer grammar, EventStream stream) {
     ...
     try{   Beat currentBeat = beats.get(0);
        for (Event ev : events) {
            int expansion = ev.getDurationEnum().getLength() - 1;
     ...
     {
         throw new EventMapConstructionException("EventStream does not fit grammar structure metrical constraints");
     }
    }
