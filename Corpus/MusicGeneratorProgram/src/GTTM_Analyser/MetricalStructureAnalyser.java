...
public class MetricalStructureAnalyser {
     ...
     * a strong beat
     * @param beatMap a map of Beat objects against Event objects
     * @param level the level to check against the time-span reduction
     * @param topTBranch the time-span reduction structure top branch.
     * @return a rating of how strongly a given event constitutes a candidate
     * for landing on a strong beat.
     */
    public double assessMPR5(List<Event> events, AttackEvent event, Map<Beat, Event> beatMap,
            int level, TimeSpanReductionBranch topTBranch)
        ...
        //MPR5e has been added at a late stage of development and certain areas of the 
        //program may not be able to use it, so at the moment it will only be used if
        //the relevant fields are set.
        if(topTBranch != null && beatMap != null){
        double rating5 = assessMPR5e(topTBranch, beatMap, level, event);
        return ((rating1 * MPR5A_WEIGHT) + (rating2 * MPR5B_WEIGHT) + (rating3 * MPR5D_WEIGHT)
                + (rating4 * MPR5C_WEIGHT) + (rating5 * MPR5E_WEIGHT))
                / (MPR5A_WEIGHT + MPR5B_WEIGHT + MPR5D_WEIGHT + MPR5C_WEIGHT + MPR5E_WEIGHT);
        }
        else{
            return ((rating1 * MPR5A_WEIGHT) + (rating2 * MPR5B_WEIGHT) + (rating3 * MPR5D_WEIGHT)
     ...
     * metrical structure given the context of Event objects.
     *
     * This method provides an assessment based on MPR5e, by identifying whether the AttackEvent
     * forms 'a relatively long duration 
     * of a pitch in the relevant levels of the time-span reduction'. 
     * 
     * 
     * @param topBranch the top TimeSpanReduction branch of the piece
     * @param beatMap a map of Beat objects against Event objects
     * @param level the time-span reduction level to check the durational values for
     * @param ev the AttackEvent to be assessed
     * @return a rating based on how likely the given AttackEvent is to land on
     * a strong beat. If the AttackEvent is the inception of a relatively long
     * pattern of inception, a ranking of 1.0 will be awarded. If it is not an
     * inception, or is not of a relatively long duration, return 0.0. If is an
    ...
    /**Used to find all the AttackEvent objects present at the given level of the
     * TimeSpanReduction
     * 
     * @param level the level of the time-span reduction to check for
     * @param topBranch the top TimeSpanReductionBranch of the analysis
     * @param beatMap a map of Beat objects against Event objects
     * @return the list of Event objects present at the given time span level.
     */
    ...
    private DurationEnum getMeanDuration(List<Event> events) {
        ...
        DurationEnum meanEnum = DurationEnum.TWO_BEATS;
        double meanDistance = Math.abs(meanLength - DurationEnum.TWO_BEATS.getLength());
