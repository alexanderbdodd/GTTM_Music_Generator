...
public class TimeSpanReductionAnalyser {
    ...
    public static final double CONSONANCE_WEIGHT = 1.0;
    public static final double TONIC_RELATION_WEIGHT = 1.0;

    public static final double TSPR2_WEIGHT = 1.0;
    ...
    /**
     * Generates a rating based on the combination of TSPR2 and TSPR3 rules, reflecting the
     * weightings of the different rules.
     ...
     * relation according to TSPR2 and TSPR3 combined. A rating towards 0.0 indicates a very weak
    ...
    public double assessConsonance(AttackEvent parent, AttackEvent child) {
        ...
        if (parent.getClass() == AttackEvent.class && child.getClass() == AttackEvent.class) {
            if (parent.getKeyLetterEnum() == child.getKeyLetterEnum()) {
                return 1;
            }
            ...
            if (parent.compare(parent, child) == -1) {
                ...
                for (IntervalConsonantRanking i : IntervalConsonantRanking.values()) {
                    if (IntervalConstructor.getInstance()
                            .getKeyAtIntervalDistance(parent.getKeyLetterEnum(), i.getInterval(), true).getKeyPosition()
                            == child.getKeyLetterEnum().getKeyPosition()) {
                        return i.getRanking();
                    }
                }
    ...
    public double assessTonicRelation(AttackEvent child, Scale localScale) {
        ...
        {
            if (localScale.getTonic() == child.getKeyLetterEnum()) {
                return 1;
            }
            if (localScale.getDominant() == child.getKeyLetterEnum()) {
                return 0.95;
            }
            if (localScale.getSubtonic() == child.getKeyLetterEnum()) {
                return 0.85;
            }
            if (localScale.getSupertonic() == child.getKeyLetterEnum()) {
                return 0.7;
            }
            if (localScale.getSubdominant() == child.getKeyLetterEnum()) {
                return 0.65;
            }
            if (localScale.getMediant() == child.getKeyLetterEnum()) {
                return 0.5;
            }
            if (localScale.getSubmediant() == child.getKeyLetterEnum()) {
                return 0.4;
            }
