...
import GTTM_Analyser.GroupingStructureAnalyser;
...
public class DurationsGenerator extends Generator {
    ...
    /**
     * Creates a map of beats against duration solutions, each map is mapped
     * against an Integer indicating how much each solution matches the metrical
     * structure solution. Solutions are determined using the grouping structure
     * constraints of the grammar
     *
     * @param grammar the grammar for use in generating the durational
     * candidates
     * @param candidateChains the candidate pitch chains for use in generating
     * durational candidates
     * @return a map of Integers against a map of Beat against Event objects.
     * The event objects represent candidate musical Events constructed using
     * the grouping durational constraints, the candidate pitch chains and the
     * metrical structure durations solution. Each Integer represents how far
     * each beat-event map matches the metrical structure constraints
     */
    public List<Pair<Integer, Map<Beat, Event>>> applyGroupingConstraints(GrammarContainer grammar,
        ...
        Map<Beat, Event> durationsMap = getCandidateDurationStreams(grammar);
                ...
                //fill in the unfilled beats
                    ...
                    if (lastDuration == null) {
                        if (durationsMap.get(b) == null) {
                            ...
                            newMap.put(b, DurationEnum.ONE_BEAT);
                        } else {
    ...
    private void recursivelyCheckBoundaryPreferences(Map<Beat, DurationEnum> eventMap,
        ...
        if (pos < beats.size()) {
            ...
            for (DurationEnum duration : DurationEnum.values()) {
                ...
                int distanceExpansion = AnalyserUtils.getInstance().distanceToNextPitch(beat, pitchBeats);
     ...
     * preference rules.
     *
     * @param grammar the grammatical structure with which to assess the
     * durational structure
     * @return a map of Beat objects against durational values which represents
     * the durational structure which meets the metrical preference rules
     * constraints
     */
    private Map<Beat, Event> getCandidateDurationStreams(GrammarContainer grammar) {
...
/* This was a last minute refactoring attempt to allow the program to support more than 5 beats, but 
    at the moment it leads the program to crash at various stages. It can work, but it is temperamental
    ...
    private Map<Beat, Event> adjustPitchDurations(List<Beat> pitchBeats) {
        ...
        for (Beat b : pitchBeats) {
            ...
            int distanceNextPitch = AnalyserUtils.getInstance().distanceToNextPitch(b, pitchBeats);

            //if the pitch is of average strength
            if (averages.contains(strength)
                    && distanceNextPitch >= durationToBeat(DurationEnum.values()[DurationEnum.values().length / 2])) {
                pitchEvent = new RestEvent(DurationEnum.values()[DurationEnum.values().length / 2]);
            } else {
                //if the strength is average, but the distance to the next beat is smaller
                //than the duration of the beat, find the largest possible duration
                //below the average that will fit
                if (averages.contains(strength)
                        && distanceNextPitch < durationToBeat(DurationEnum.values()[DurationEnum.values().length / 2])) {
                    boolean fitting = false;
                    int pos = 1;
                    while (!fitting && (DurationEnum.values().length / 2) - pos > 0) {
                        int distance = durationToBeat(DurationEnum.values()[(DurationEnum.values().length / 2) - pos]);
                        if (distanceNextPitch >= distance) {
                            fitting = true;
                            pitchEvent = new RestEvent(DurationEnum.TWO_BEATS);
                        } else {
                            pos++;
                        }
                    }
                    ...
                    if (!fitting) {
                        pitchEvent = new RestEvent(DurationEnum.ONE_BEAT);
                    }
                    ...
                    if (strength < averages.get(0)) {
                        ...
                        if (distance > 1 || distanceNextPitch < durationToBeat(DurationEnum.TWO_BEATS)) {
                            pitchEvent = new RestEvent(DurationEnum.ONE_BEAT);
                        } else {
                            DurationEnum currentEnum = DurationEnum.values()[1];
                            DurationEnum nextEnum = DurationEnum.values()[2];
                            int pos = 3;
                            while (distanceNextPitch < durationToBeat(nextEnum)
                                    && currentEnum != DurationEnum.values()[DurationEnum.values().length / 2]) {
                                currentEnum = nextEnum;
                                nextEnum = DurationEnum.values()[pos];
                                pos++;
                            }
                            pitchEvent = new RestEvent(currentEnum);
                        }
                        ...
                        while (distanceNextPitch < durationToBeat(DurationEnum.values()[DurationEnum.values().length - pos]) 
                                && distance > 1) {
                            pos++;
                        }

                        pitchEvent = new RestEvent(DurationEnum.values()[DurationEnum.values().length - pos]);
                ...
                for (Beat b : pitchBeats) {
                    ...
                    int spanDistance = events.get(b).getDurationEnum().getLength() - 1;
...
*/
    ...
    private Map<Beat, Event> adjustPitchDurations(List<Beat> pitchBeats) {
        ...
        for (Beat b : pitchBeats) {
            ...
            //if the pitch is of average strength
            if (averages.contains(strength)
                    && distanceNextPitch >= durationToBeat(DurationEnum.THREE_BEATS)) {
                pitchEvent = new RestEvent(DurationEnum.THREE_BEATS);
            } else {
                //if the strength is average, but the distance to the next beat is smaller
                //than the duration of the beat, find the largest possible duration
                //below the average that will fit
                if (averages.contains(strength)
                        && distanceNextPitch < durationToBeat(DurationEnum.THREE_BEATS)) {
                    if (distanceNextPitch >= durationToBeat(DurationEnum.TWO_BEATS)) {
                        pitchEvent = new RestEvent(DurationEnum.TWO_BEATS);
                    } else {
                        pitchEvent = new RestEvent(DurationEnum.ONE_BEAT);
                    }
                    ...
                    if (strength < averages.get(0)) {
                        ...
                        if (distance > 1 && distanceNextPitch >= durationToBeat(DurationEnum.FIVE_BEATS)) {
                            pitchEvent = new RestEvent(DurationEnum.FIVE_BEATS);
                        } else if (distanceNextPitch >= durationToBeat(DurationEnum.FOUR_BEATS)) {
                            pitchEvent = new RestEvent(DurationEnum.FOUR_BEATS);
                        } else {
                            if (distanceNextPitch >= durationToBeat(DurationEnum.THREE_BEATS)) {
                                pitchEvent = new RestEvent(DurationEnum.THREE_BEATS);
                            } else {
                                if (distanceNextPitch >= durationToBeat(DurationEnum.TWO_BEATS)) {
                                    pitchEvent = new RestEvent(DurationEnum.TWO_BEATS);
    ...
    private Map<Beat, Event> fillEvents() {
        ...
        for (Beat b : getmContainer().getMetricalBeatsList()) {
            ...
            RestEvent rest = new RestEvent(DurationEnum.ONE_BEAT);
